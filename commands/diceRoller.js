 ...
 *   Rewritten 2021-7-25
 *   Silas (ExVacuum)
 *   Tweaked to fit new API 2022-01-31
 *   Ian
 */
...
const STATUS = {
  keptMoreThanRolled: {
    flag: 0b0001,
    message: (args) =>
      `Why do you want me to keep the top ${args[ARG.keep]} of ${
        args[ARG.num]
      } dice roll${args[ARG.num] > 1 ? "s" : ""}?\n`,
  },
  keptZero: {
    flag: 0b0010,
...
function rollDice(dice, message) {
  ...
  args.forEach((arg, i) => {
    args[i] = Number.parseInt(args[i], 10);
  });
  let results = [];
  ...
  returnMessage = returnMessage.concat(
    `${args[ARG.keep] ? `Top ${args[ARG.keep]} of ` : ""}${args[ARG.num]} ${
      args[ARG.sides]
    }-sided di${args[ARG.sides] > 1 ? "ce" : "e"} roll${
      args[ARG.num] > 1 ? "s" : ""
    }:\n`
  );
  ...
  execute(interaction, client) {
    ...
    if (diceRegex.test(diceString)) {
      ...
      diceSets.forEach((dice) => {
        returnMessage = rollDice(dice, returnMessage);
      });
    }
    interaction.reply(
      returnMessage.length <= 2000
        ? returnMessage
        : "This command would have generated way too much output, please try something else."
    );
  },
};
